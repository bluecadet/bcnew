...
function prepareComponents(config, components) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if ( !components ) {
      ...
      components.forEach(component => {
        fsx.copySync(component, `${config.themePath}/${config.project}/fractal/components`, {overwrite: true});
      });
    }
  });
}
...
function prepareRoot(config) {
  ...
  return new Promise((resolve, reject) => {
    ...
    let gulpConfigFile = `${rootSrc}/.gulpconfig.js`;
...
module.exports = function(config, components) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (components) {
      console.log('You Did It', components);
    }
    ...
    dlrepo('bluecadet/bc-base-themes', `${config.tempDir}`, { clone: true }, err => {
      ...
      prepareTheme(config)
        .then(() => {
          prepareComponents(config, components)
          .then(() => {
