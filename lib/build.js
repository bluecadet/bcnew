...
module.exports = function(config) {
  ...
  dlrepo('bluecadet/bc-base-themes', `${config.tempDir}`, { clone: true }, err => {
        ...
        .then(() => {
          prepareRoot(config)
            .then(() => {
              ...
              // let installer = exec(`cd ${config.curPath} && npm install`);
    ...
    // // Download Gulpfile
    // dlrepo('bluecadet/bluecadet_gulpfiles#v2', `${config.tempDir}/gulp`, { clone: true }, err => {
    //   if (err) {
    //     console.log(chalk.red(`ERROR @ downloading gulp`));
    //     throw Error(err);
    //   }

    //   console.log(chalk.blue('Preparing assets'));
